...
var _bufferMatch = function(buffer, contents, offset) {
	if(contents.length + offset > buffer.length) {
		throw new Error('Invalid argument');
	}
	for(var i = offset; i < contents.length; i++) {
...
var _bufferIndexOf = function(buffer, str) {
	...
	var end = buffer.length - contents.length + 1;
	for(var i = 0; i < end; i++) {
