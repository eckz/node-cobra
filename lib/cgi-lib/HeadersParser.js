...
var HeadersParser = function() {
	...
	this.headersBuffer = new Buffer(1024);
	this.headersOffset = 0;
};

nodeUtil.inherits(HeadersParser, EventEmitter);

HeadersParser.prototype._ensureBufferLength = function(l) {
	if(this.headersBuffer.length < l) {
		var oldBuffer = this.headersBuffer;
		this.headersBuffer = new Buffer(l + 1024);
		oldBuffer.copy(this.headersBuffer);
	}
};

HeadersParser.prototype.write = function(data) {
	if(!this.headersParsed) {
		this._ensureBufferLength(this.headersOffset + data.length);
